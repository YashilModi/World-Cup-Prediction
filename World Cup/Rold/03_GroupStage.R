#Goal: simulate one group fully and produce a correctly-ranked standings table.
source("R/02_Match_Engine.R")   # gives us simulate_match(), rate, teams, etc.

simulate_group <- function(group_letter) {
  
  # Step 1: this group's teams and fixtures
  ...
  # Step 2: a final score for every match
  ...
  # Step 4: sort by points, then goal difference, then goals scored
  standings <- standings[order(-standings$points, -standings$gd, -standings$gf), ]
  
  # Step 5: hand back the finished table
  return(standings)
}