# ===== Headline 6: Monte Carlo → championship probabilities =====
source("R/04_Knockout_Bracket.R")
# Step 1: one whole tournament → a champion
...
# Step 3: counts → probabilities, with names
probs <- sort(table(champions) / n_sims, decreasing = TRUE)
prob_table <- data.frame(
  team = teams$team_name[match(as.integer(names(probs)), teams$team_id)],
  prob = round(as.numeric(probs), 3)
)
head(prob_table, 15)

# Step 4: bar chart of the top 15, saved to disk
...
barplot(
  rev(top15$prob),                        # rev() puts the biggest bar at the top
